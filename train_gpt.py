...
# ---------------------------------------------------------------------------
# Self-contained data download fallback
# If dataset/tokenizer files are not present, download them from HuggingFace.
# This makes the training script work on fresh RunPod pods without infra help.
# ---------------------------------------------------------------------------
def _ensure_data() -> tuple[str, str]:
    """Return (data_path, tokenizer_path), downloading from HF if needed."""
    _HF_REPO = "willdepueoai/parameter-golf"
    _HF_PREFIX = "datasets"
    ...
    candidates = [
        script_dir / "data" / "datasets" / _VARIANT,
        Path("/data/datasets") / _VARIANT,
        Path("/workspace/data/datasets") / _VARIANT,
        Path("/workspace/datasets") / _VARIANT,
        Path("/opt/datasets") / _VARIANT,
    ]
    data_path = os.environ.get("DATA_PATH", "")
    if data_path:
        candidates.insert(0, Path(data_path))

    found_data = None
    for c in candidates:
        if c.is_dir() and list(c.glob("fineweb_train_*.bin")):
            found_data = c
            break

    # Candidate tokenizer paths
    tok_path_env = os.environ.get("TOKENIZER_PATH", "")
    tok_candidates = [
        Path(tok_path_env) if tok_path_env else None,
        script_dir / "data" / "tokenizers" / "fineweb_1024_bpe.model",
        Path("/workspace/data/tokenizers/fineweb_1024_bpe.model"),
    ...
    # Download if needed
    if found_data is None or found_tok is None:
        try:
        ...
        def _dl(remote_rel: str, local_dest: Path):
        ...
        if found_tok is None:
            tok_dir = dl_root / "tokenizers"
            tok_dir.mkdir(parents=True, exist_ok=True)
            print("Downloading tokenizer from HF...", flush=True)
            for tf in _TOKENIZER_FILES:
                _dl(f"{_HF_PREFIX}/tokenizers/{tf}", tok_dir / tf)
            found_tok = tok_dir / "fineweb_1024_bpe.model"
            print(f"Tokenizer download complete: {found_tok}", flush=True)

    # Set env vars so Hyperparameters picks them up
    os.environ.setdefault("DATA_PATH", str(found_data))
...
if _is_rank0():
    _ensure_data()
    # Write sentinel so other ranks know data is ready
    _sentinel = Path(__file__).resolve().parent / "data" / ".data_ready"
    _sentinel.parent.mkdir(parents=True, exist_ok=True)
    _sentinel.write_text("ok")
else:
    # Wait for rank 0 to finish downloading (up to 600s)
    _sentinel = Path(__file__).resolve().parent / "data" / ".data_ready"
    _t0 = time.time()
    while not _sentinel.exists() and (time.time() - _t0) < 600:
        time.sleep(2)
...
class Hyperparameters:
    ...
    lawa_enabled = bool(int(os.environ.get("LAWA_ENABLED", "0")))  # LAWA k=10 regressed +0.0024 bpb vs EMA (1.1612 vs 1.1588)
    ...
    gptq_clip_range = int(os.environ.get("GPTQ_CLIP_RANGE", 31))  # int6: [-31, 31]; brotli-11 keeps artifact under 16MB
    ...
    # TTT: Legal Score-First Test-Time Training (PR #672, #1130)
...
if triton is not None:
    ...
    @triton.jit
    def _leaky_relu_sq_forward_kernel(
    ...
    @triton.jit
    def _leaky_relu_sq_backward_kernel(
        x_ptr, grad_out_ptr, grad_in_ptr, n_elements, BLOCK_SIZE: tl.constexpr
    ):
        pid = tl.program_id(0)
        offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
        mask = offsets < n_elements
...
class TritonLeakyReluSqFn(torch.autograd.Function):
    ...
    def backward(ctx, grad_out: Tensor) -> tuple[Tensor]:
        ...
        grid = lambda meta: (triton.cdiv(n_elements, meta["BLOCK_SIZE"]),)
        _leaky_relu_sq_backward_kernel[grid](
            x, grad_out_contig, grad_in, n_elements, BLOCK_SIZE=1024
        )
        return (grad_in,)
...
class TrainNgramTracker:
    ...
    def __init__(
        self, vocab_size: int, device: torch.device, complement_alpha: float = 0.5
    ):
        self.V = vocab_size
        self.alpha = complement_alpha
        self.bi_counts = torch.zeros(
            vocab_size, vocab_size, device=device, dtype=torch.float32
        )
...
def read_data_shard_header(file: Path) -> dict[str, int]:
    ...
    if (
        header.size != SHARD_HEADER_WORDS
...
def load_data_shard(file: Path) -> Tensor:
    ...
    tokens_np = np.fromfile(
        file, dtype=SHARD_TOKEN_DTYPE, count=num_tokens, offset=SHARD_HEADER_BYTES
    )
...
class CoprimeDistributedTokenLoader:
            ...
            self.shards.append(
                {
                    ...
                    "stride": choose_coprime_stride(
                        num_blocks, seed * 29 + shard_idx * 7 + 1
                    ),
                }
            )
        if not self.shards:
    ...
    def _sample_sequences(self, shard_idx: int, count: int) -> Tensor:
        ...
        block_ids = (
            offset + (visits + torch.arange(count, dtype=torch.int64)) * stride
        ) % num_blocks
    ...
    def next_batch(
        ...
        if seq_len != self.seq_len:
            raise ValueError(
                f"Coprime loader was built for seq_len={self.seq_len}, got {seq_len}"
            )
        local_tokens = global_tokens // (self.world_size * grad_accum_steps)
        ...
        shard_start = (
            (hold_idx * self.world_size) + self.rank
        ) * self.batch_shard_stride
        chunks: list[Tensor] = []
        for shard_slot in range(active_shards):
            ...
            if count <= 0:
                continue
            shard_idx = (
                shard_start + shard_slot * self.batch_shard_stride
            ) % self.num_shards
...
def build_train_loader(
    args: Hyperparameters, rank: int, world_size: int, device: torch.device
):
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self._calib_save_y = (
            None  # set during GPTQ calibration to capture pre-O-proj attn output
        )
    ...
    def forward(
        ...
        if flash_attn_3_func is not None:
            ...
            y = F.scaled_dot_product_attention(qh, kh, vh, is_causal=True).transpose(
                1, 2
            )
        if self.use_xsa:
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        ...
        self._calib_save_act = (
            None  # set during GPTQ calibration to capture pre-down-proj activation
        )
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn_scale = nn.Parameter(
            torch.full((dim,), attn_scale_init, dtype=torch.float32)
        )
        self.mlp_scale = nn.Parameter(
            torch.full((dim,), mlp_scale_init, dtype=torch.float32)
        )
...
class GPT(nn.Module):
    ...
    def forward_hidden(self, input_ids: Tensor) -> Tensor:
        """Return last-layer hidden states BEFORE final_norm (for SLOT)."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        ...
        return x  # pre-final_norm hidden states

    def logits_from_hidden(self, h: Tensor) -> Tensor:
        """Compute logits from hidden states (used by SLOT and forward_logits)."""
        x = self.final_norm(h)
        if self.tie_embeddings:
...
def _ngram_bulk_update(
    val_np, start, end, ctx_tables, full_tables, min_order, max_order, primes, mask
):
    ...
    for order in range(min_order, max_order + 1):
        ...
        tgt = t[order - 1 :]
        full_key = ((ctx_hash ^ (tgt * primes[ctx_width % len(primes)])) & mask).astype(
            np.int64
        )
        ctx_tables[order] += np.bincount(
            ctx_key, minlength=len(ctx_tables[order])
        ).astype(np.uint32)
        full_tables[order] += np.bincount(
            full_key, minlength=len(full_tables[order])
        ).astype(np.uint32)
...
def eval_val_sliding_hashed_ngram(
    ...
    if args.ngram_order_mults_str:
        _fixed_order_mults = np.array(
            [float(x) for x in args.ngram_order_mults_str.split(",")], dtype=np.float64
        )
    ...
    ctx_tables = {
        n: np.zeros((buckets,), dtype=np.uint32)
    ...
    _ENT_EDGES = np.array(
        [ent_center - 1.0, ent_center + 1.0]
    )  # [2.0, 4.0] for center=3.0
    ...
    if _con:
        ...
        _c_alpha_mult = {
            n: [_WARM.get(n, 1.0)] * _TOTAL_CELLS
            for n in range(min_order, max_order + 1)
        }
    ...
    if rank == 0:
        print(
            f"ngram_eval:chunks={num_chunks} chunk_tokens={chunk_tokens} "
            f"windows={len(all_window_starts)} shared_tables=True",
            flush=True,
        )
    ...
    with torch.inference_mode():
        for ci in range(num_chunks):
                ...
                for i, ws in enumerate(batch_ws):
                    ...
                    for n in range(max_order, min_order - 1, -1):
                        ...
                        full_key = (
                            (
                                ctx_hash
                                ^ (tgt_np[v_idx] * primes[ctx_width % len(primes)])
                            )
                            & mask
                        ).astype(np.int64)
                        ...
                        if has_data.any():
                            p = np.minimum(full_counts, ctx_counts) / np.maximum(
                                ctx_counts, 1.0
                            )
                            p = np.clip(p, 0.0, 1.0)
                    ...
                    if ng_matched.any():
                        ...
                        if adaptive and args.ngram_entropy_shift:
                            ...
                            shifted_centers = ent_center - 0.25 * (
                                matched_ords - float(min_order)
                            )
                            shifted_sig = 1.0 / (
                                1.0
                                + np.exp(
                                    -ent_scale * (entropy[m_idx] - shifted_centers)
                                )
                            )
                            per_token_alpha[m_idx] = (
                                alpha_min + (alpha_max - alpha_min) * shifted_sig
                            )
                        if _fixed_order_mults is not None:
                            ...
                            mult_indices = np.clip(
                                mult_indices, 0, len(_fixed_order_mults) - 1
                            )
                            ...
                            m_cnt_bins = np.digitize(
                                _ng_ctx_count[m_idx], _CNT_EDGES
                            ).astype(np.int32)
                            for n in range(min_order, max_order + 1):
                                om = _ng_ord[m_idx] == n
                                if not om.any():
                                ...
                                for eb in range(_NUM_ENT_BINS):
                                    for cb in range(_NUM_CNT_BINS):
                                        ...
                                        mask_ecb = (
                                            om & (m_ent_bins == eb) & (m_cnt_bins == cb)
                                        )
                                        if mask_ecb.any():
                        ...
                        seg_model_p[m_idx] = (1.0 - a) * seg_model_p[m_idx] + a * p_ng[
                            m_idx
                        ]
            ...
            if _con:
                        ...
                        for cell in range(_TOTAL_CELLS):
                            if _c_hits[n][cell] >= 8:
                                ...
                                if rate > avg_rate + 0.05:
                                    _c_alpha_mult[n][cell] = min(
                                        _c_alpha_mult[n][cell] * 1.03, 2.0
                                    )
                                elif rate < avg_rate - 0.05:
                                    _c_alpha_mult[n][cell] = max(
                                        _c_alpha_mult[n][cell] * 0.97, 0.3
                                    )
                _cfired += 1
                ...
                _c_hits = {
                    n: [0] * _TOTAL_CELLS for n in range(min_order, max_order + 1)
                }
                _c_beats = {
                    n: [0] * _TOTAL_CELLS for n in range(min_order, max_order + 1)
                }
    ...
    if _con and rank == 0:
        print(
            f"cubric3d:final c_steps={_cfired} cells={_TOTAL_CELLS}x{max_order - min_order + 1}={_TOTAL_CELLS * (max_order - min_order + 1)}",
            flush=True,
        )
        for n in range(min_order, max_order + 1):
            m = _c_alpha_mult[n]
...
# --- TTT (Legal Score-First Test-Time Training) sliding eval ---
...
def eval_val_sliding_ttt(
    ...
    """Legal score-first TTT (PR #461/#672/#1130 recipe): score each chunk with
    sliding windows, then train on it. Every token scored BEFORE any update that
    could use it. Cosine LR schedule across chunks."""
    seq_len = args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    ...
    frozen_block_ids = set(
        range(min(args.ttt_freeze_blocks, len(base_model.blocks)))
    )
    ttt_params = []
    ...
    optimizer = torch.optim.SGD(
        ttt_params, lr=args.ttt_lr, momentum=args.ttt_momentum
    )
    t0 = time.perf_counter()
        ...
        with torch.inference_mode():
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                for i, ws in enumerate(batch_ws):
                    ...
                    tb = base_bytes_lut[tgt].to(torch.float64)
                    tb += (
                        has_leading_space_lut[tgt]
                        & ~is_boundary_token_lut[prev]
                    ).to(torch.float64)
                    byte_count += tb.sum()
    ...
    # Restore all params to requires_grad and eval mode
...
# --- SLOT (Stochastic Logit Overlay at Test-time) sliding eval ---
...
def eval_val_sliding_slot(
    ...
    """Sliding window evaluation with SLOT: per-batch learnable hidden delta.

    Protocol (PR #1172/PR #1176):
    1. Forward pass to get hidden states H (before final_norm)
    2. Create delta [bsz, 1, dim] initialized to zeros
    3. Run slot_steps AdamW steps optimizing CE(logits_from_hidden(H + delta), targets)
    4. Score with optimized delta
    """
    seq_len = eval_seq_len or args.train_seq_len
    model_dim = base_model.tok_emb.weight.shape[1]
    total_tokens = val_tokens.numel() - 1
    window_starts = [
        ws
        for ws in range(0, total_tokens, stride)
    ...
    with torch.inference_mode(False):
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            # Phase 1: get hidden states (no grad needed for this)
            with torch.no_grad():
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    H = base_model.forward_hidden(x_batch)
                H = H.detach().float()  # [bsz, seq_len, dim]

            # Phase 2: SLOT optimization — learn per-batch delta
            delta = torch.zeros(bsz, 1, model_dim, device=device, dtype=torch.float32, requires_grad=True)
            slot_opt = torch.optim.AdamW([delta], lr=args.slot_lr, weight_decay=args.slot_wd)
            for _ in range(args.slot_steps):
                slot_opt.zero_grad()
                logits = base_model.logits_from_hidden((H + delta).bfloat16())
            ...
            # Phase 3: Score with optimized delta (detached)
            with torch.no_grad():
                logits_final = base_model.logits_from_hidden((H + delta.detach()).bfloat16())
                nll = F.cross_entropy(
                    logits_final.float().reshape(-1, logits_final.size(-1)),
                    y_batch.reshape(-1),
                    reduction="none",
                ).reshape(bsz, seq_len)
            ...
            n_batches += 1
            if rank == 0 and (n_batches % 50 == 0 or bi + batch_seqs >= len(my_windows)):
                elapsed = time.perf_counter() - t0
                cur_bpb = (
                    (loss_sum.item() / max(token_count.item(), 1.0))
                    / math.log(2.0)
                    * (token_count.item() / max(byte_count.item(), 1.0))
                    if token_count.item() > 0
                    else 0.0
                )
                print(
                    f"slot_eval:batch {n_batches} bpb={cur_bpb:.6f} t={elapsed:.0f}s",
                    flush=True,
                )
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim >= 2:
        ...
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            rc = (
                torch.quantile(tw.abs(), pct, dim=1)
                if pct < 1.0
                else tw.abs().amax(dim=1)
            )
        ...
        return best_q.reshape(t32.shape).contiguous(), best_s.reshape(
            t32.shape[:-1]
        ).contiguous()
...
def _gptq_impl(
    W: Tensor, H: Tensor, clip_range: int, block_size: int
) -> tuple[Tensor, Tensor]:
    ...
    for j in range(n_in):
        ...
        if j + 1 < n_in:
            W[:, j + 1 :] -= err[:, None] * (
                H_inv[j, j + 1 :] / H_inv[j, j].clamp_min(1e-8)
            )
...
def quantize_int6_gptq(
    ...
    try:
        return _gptq_impl(
            weight.float().clone(), hessian.float().clone(), clip_range, block_size
        )
    except Exception:
        return quantize_int6_per_row(weight, clip_range)
...
def quantize_state_dict_int6_gptq(
    ...
    for name, t in state_dict.items():
        ...
        stats["n_quant"] += 1
        stats["quant_bytes"] += (
            q.numel() * q.element_size() + s.numel() * s.element_size()
        )
...
def main() -> None:
    ...
    if args.ngram_eval_order >= 2:
        log0(
            f"ngram_eval:order={args.ngram_eval_order} alpha={args.ngram_eval_alpha} min_count={args.ngram_eval_min_count} buckets={args.ngram_eval_buckets}"
        )
    ...
    if args.complement_alpha > 0:
        tracker = TrainNgramTracker(
            args.vocab_size, device, complement_alpha=args.complement_alpha
        )
    ...
    if args.gptq_enabled:
        ...
        if master_process:
            ...
            base_model.eval()
            hessians = _collect_hessians(
                base_model, _get_calib_batch, args.gptq_calib_batches, device
            )
            log0(
                f"gptq:hessians_collected time={1000 * (time.perf_counter() - t_gptq_start):.0f}ms"
            )
            # Quantize state dict
    ...
    if args.skip_final_eval:
        ...
        if args.eval_stride != 64 and 64 < sw_seq_len:
            ...
            log0(
                f"final_sliding_window_s64_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f}"
            )
        # TTT eval — legal score-first test-time training (modifies model weights)
        if args.ttt_enabled:
            torch.cuda.synchronize()
            ...
            log0(
                f"final_sliding_window_ttt_exact val_loss:{ttt_val_loss:.8f} val_bpb:{ttt_val_bpb:.8f}"
            )
        # SLOT eval — per-batch hidden state delta optimization (runs on TTT-modified model if TTT enabled)
        if args.slot_enabled:
            ...
            torch.cuda.synchronize()
            log0(
                f"final_sliding_window_slot val_loss:{slot_val_loss:.4f} val_bpb:{slot_val_bpb:.4f} "
                f"slot_lr:{args.slot_lr} slot_steps:{args.slot_steps} "
                f"eval_time:{1000.0 * (time.perf_counter() - t_slot):.0f}ms"
            )
            log0(
                f"final_sliding_window_slot_exact val_loss:{slot_val_loss:.8f} val_bpb:{slot_val_bpb:.8f}"
            )
        if args.ngram_eval_order >= 2:
            if distributed:
                dist.barrier()
