...
from agents.shared import (
    RESEARCH_QUEUE_PATH as _RESEARCH_QUEUE,
    mark_fulfilled,
    read_unfulfilled,
)
...
def _handle_promotion(request: dict) -> None:
    ...
    # 4. Critic gate — deterministic + LLM checks on the diff
    from research.critic import run_critique

    try:
        critic_verdict = run_critique()
    except Exception as exc:
        print(f"[orchestrate] Critic gate error (non-blocking): {exc}")
        critic_verdict = "pass"

    if critic_verdict == "block":
        print("[orchestrate] BLOCKED by critic gate")
        return

    # 5. Contamination check — AST-based validation data leakage detection
    from compute.contamination import check_data_overlap

    script_path = Path("train_gpt.py")
    contamination_result = check_data_overlap(script_path)
    print(f"[orchestrate] Contamination check: {contamination_result.status} — {contamination_result.detail}")
    if contamination_result.status == "block":
        print("[orchestrate] BLOCKED by contamination check")
        return
    ...
    # Auto-set early-abort threshold if not explicitly configured
    if "EARLY_ABORT_BPB" not in forwarded_env and current_bpb > 0:
        # 1% headroom above current best — abort at step 1000 if worse than this
        abort_threshold = current_bpb * 1.01
        forwarded_env["EARLY_ABORT_BPB"] = f"{abort_threshold:.4f}"
        print(f"[orchestrate] Early-abort threshold set to {abort_threshold:.4f} (current best + 1%)")
    ...
    # Detect early-abort from run log
    run_log_text = (run_dir / "run.log").read_text() if (run_dir / "run.log").exists() else ""
    early_aborted = "early_abort:" in run_log_text
    ...
    if early_aborted:
        status = "early_abort"
        print("[orchestrate] Run was early-aborted at step 1000 (below threshold)")
    elif has_failures(post_checks):
        status = "flagged"
        print(
    ...
    # Update best H100 bpb for dynamic early-abort threshold calibration
    val_bpb = result.get(_KEY_VAL_BPB)
    if val_bpb and status == "keep":
        budget.update_best_bpb(float(val_bpb))

    print(f"[orchestrate] Done. val_bpb={val_bpb} cost=${cost:.2f}")
...
def _run_supervisor() -> None:
    ...
    _pending_research_timestamps: list[str] = []  # timestamps of requests being processed
    ...
    while True:
        ...
        if not _check_agent_alive(experiment_proc):
            ...
            # Generate compact decision state before restarting agent
            try:
                from agents.decision_state import write_decision_state
                ds_path = write_decision_state()
                print(f"[orchestrate] Decision state written to {ds_path}")
            except Exception as exc:
                print(f"[orchestrate] Decision state generation failed: {exc}")
        ...
        if research_proc is not None and not _check_agent_alive(research_proc):
            ...
            # Mark reactive requests as fulfilled on successful completion
            if rc == 0 and _pending_research_timestamps:
                mark_fulfilled(_RESEARCH_QUEUE, _pending_research_timestamps)
                _pending_research_timestamps = []
            if _RESEARCH_ON_DEMAND:
        ...
        if _RESEARCH_ON_DEMAND and research_proc is None:
            unfulfilled = read_unfulfilled(_RESEARCH_QUEUE)
            if unfulfilled:
                print(
                    f"[orchestrate] Research queue has {len(unfulfilled)} unfulfilled request(s). Spawning research agent..."
                )
                _pending_research_timestamps = [
                    e.get("timestamp", "") for e in unfulfilled
                ]
                research_proc = _launch_agent(_RESEARCH_AGENT_PROMPT, "research-agent")
...
def main() -> None:
    ...
    if args.budget_status:
        ...
        result = run_tournament(config, source_file=Path("train_gpt_mlx.py"))
